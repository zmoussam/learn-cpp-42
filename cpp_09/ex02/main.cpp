...
/*   Updated: 2023/07/31 04:31:04 by zmoussam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int ac, char **av)
{
    ...
    if (ac >= 2)
    {
        ...
        end_time = clock();
        list_process = (double)(end_time - start_time) / (CLOCKS_PER_SEC);
        if (!lst.empty())
        {
            start_time = clock();
            vec = __fordJohnson_sort_vector(av);
            end_time = clock();
            vector_process = (double)(end_time - start_time) / (CLOCKS_PER_SEC);
            printBeforeSort(av);
            printSortedList(lst);
